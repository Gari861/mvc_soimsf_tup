...
{
    [Authorize]
    public class EstadosDeAfiliadosController : Controller
    {
        private readonly HttpClient _client;
        private readonly string _apiBaseUrl;

        public EstadosDeAfiliadosController()
        {
            _client = new HttpClient();
            //_apiBaseUrl = "https://localhost:7086/api/EstadosDeAfiliados";
            _apiBaseUrl = "https://soimsf.somee.com/api/EstadosDeAfiliados";
        }
        ...
        // GET: EstadosDeAfiliadosController
        ...
        // GET: EstadosDeAfiliadosController/Details/5
        public async Task<ActionResult> Details(int id)
        {
            AddJwtToken(); // Agregar token JWT

            EstadosDeAfiliadosViewModel estado = null;
            HttpResponseMessage response = await _client.GetAsync($"{_apiBaseUrl}/{id}");

            if (response.IsSuccessStatusCode)
            {
                string data = await response.Content.ReadAsStringAsync();
                estado = JsonConvert.DeserializeObject<EstadosDeAfiliadosViewModel>(data);
            }
            else if (response.StatusCode == System.Net.HttpStatusCode.Unauthorized)
            {
                return RedirectToAction("Login", "Authentication");
        ...
        // GET: EstadosDeAfiliadosController/Create
        ...
        // POST: EstadosDeAfiliadosController/Create
        [Authorize(Roles = "Admin")]
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Create(EstadosDeAfiliadosViewModel estado)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        TempData["AlertMessage"] = "¡Estado de Afiliado creado con éxito!";
        ...
        // GET: EstadosDeAfiliadosController/Edit/5
        ...
        // POST: EstadosDeAfiliadosController/Edit/5
        [Authorize(Roles = "Admin,Manager")]
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit(EstadosDeAfiliadosViewModel estado)
        {
            if (!ModelState.IsValid)
            {
                return View(estado);
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    TempData["AlertMessage"] = "¡Estado de Afiliado editado con éxito!";
        ...
        // GET: EstadosDeAfiliadosController/Delete/5
        [Authorize(Roles = "Admin")]
        ...
        // POST: EstadosDeAfiliadosController/Delete/5
        [Authorize(Roles = "Admin")]
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Delete(int id, IFormCollection collection)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    TempData["AlertMessage"] = "¡Estado de Afiliado eliminado con éxito!";
